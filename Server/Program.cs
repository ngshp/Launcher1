...
// Gunakan async Main agar Task.Run dan await bisa berjalan dengan stabil
await Task.Run(() => {
...
|_| \_|\____|_|   |____/  |_____\___/ \____/_/   \_\_____|_| |_|\___/|____/ |_|  
    ...
    try { if(File.Exists("ban_hwid_ip.txt")) foreach(var l in File.ReadAllLines("ban_hwid_ip.txt")) banlist.Add(l); } catch {}
    ...
    while(true) {
        ...
        // Menggunakan _ = untuk menghindari warning "call is not awaited"
        _ = Task.Run(async () => {
            try {
                var s = client.GetStream(); 
                var w = Encoding.UTF8.GetBytes("WELCOME NGPB ENTERPRISE v1.0.105 RED Yard 002 7R 001 02:45 HP 100 AP 100 28/120 AUG A3\n");
                await s.WriteAsync(w, 0, w.Length); 
