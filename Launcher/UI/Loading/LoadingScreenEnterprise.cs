...
// FIX AMBIGUOUS ProgressBar - Pakai WPF only Bos!
using ProgressBar = System.Windows.Controls.ProgressBar;
using MessageBox = System.Windows.MessageBox;

namespace PBNG.Launcher.UI.Loading
{
    // LAYER 1 LOADING SCREEN - Jika di bypass = AUTO CLOSE CLIENT - Cyberpunk Blue 0A0A1A -> 00E5FF
    public class LoadingScreenLayer1 : Window
    {
        ...
        UIElement BuildUI() {
            ...
            var subtitle = new TextBlock { Text="LOADING SCREEN LAYER 1 - SECURITY CHECK - RED Yard 8v8", Foreground=Brushes.White, FontSize=14, FontFamily=new FontFamily("Consolas"), HorizontalAlignment=HorizontalAlignment.Center, Margin=new Thickness(0,10,0,30) };
            ...
            var warning = new TextBlock { Text="⚠️ BYPASS DETECTED = AUTO CLOSE CLIENT - HWID: "+Environment.MachineName, Foreground=new SolidColorBrush(Color.FromRgb(255,68,68)), FontSize=10, FontFamily=new FontFamily("Consolas"), HorizontalAlignment=HorizontalAlignment.Center, Margin=new Thickness(0,20,0,0) };
        ...
        async Task StartLoading() {
            string[] steps = new string[] {
                "Checking Anti-Cheat Enterprise 4 Layer...",
                "Verifying Game Integrity ngpb.exe 60 MB...",
                "Scanning Memory for Cheat Engine...",
                "Loading RED Yard Map 8v8 Industrial...",
                "Loading AUG A3 28/120 + M4A1 AK-47...",
                "Security Layer 1 - PASSED - 127.0.0.1:39190"
            };
            for(int i=0;i<steps.Length;i++) {
                status.Text = steps[i]; bar.Value = (i+1)*100.0/steps.Length;
                await Task.Delay(600);
                if(bar.Value<0 || bar.Value>100) { IsBypassed=true; MessageBox.Show("SECURITY BREACH: Loading Screen Layer 1 Bypass Detected!\nAuto Close Client!\nWeb: https://ngpb.nhg.one", "PBNG Anti-Cheat Enterprise", MessageBoxButton.OK, MessageBoxImage.Stop); Environment.Exit(0); }
            }
            IsPassed = true; await Task.Delay(300); Close();
        }

        protected override void OnClosing(System.ComponentModel.CancelEventArgs e) {
            if(!IsPassed &&!IsBypassed) { IsBypassed=true; MessageBox.Show("BYPASS LAYER 1 = AUTO CLOSE CLIENT!", "Anti-Cheat"); Environment.Exit(0); }
            base.OnClosing(e);
        }
    }

    // LAYER 2 LOADING SCREEN - Jika di bypass = AUTO BAN HWID + IP PERMANEN - Cyberpunk Pink 1A0033 -> FF0080
    public class LoadingScreenLayer2 : Window
    {
        ...
        UIElement BuildUI() {
            ...
            var subtitle = new TextBlock { Text="LOADING SCREEN LAYER 2 - HWID + IP PROTECTION - 0.0.0.0:39190 VPS", Foreground=Brushes.White, FontSize=14, FontFamily=new FontFamily("Consolas"), HorizontalAlignment=HorizontalAlignment.Center, Margin=new Thickness(0,10,0,30) };
            ...
            var warning = new TextBlock { Text="⛔ BYPASS LAYER 2 = AUTO BAN HWID + IP PERMANEN! - "+Environment.MachineName, Foreground=new SolidColorBrush(Color.FromRgb(255,68,68)), FontSize=11, FontWeight=FontWeights.Bold, FontFamily=new FontFamily("Consolas"), HorizontalAlignment=HorizontalAlignment.Center, Margin=new Thickness(0,20,0,0) };
            stack.Children.Add(title); stack.Children.Add(subtitle); stack.Children.Add(status); stack.Children.Add(bar); stack.Children.Add(warning);
            border.Child = stack; grid.Children.Add(border); return grid;
        }
        ...
        async Task StartLoading() {
            string[] steps = new string[] {
                "Generating HWID: "+Environment.MachineName,
                "Checking IP Address: 127.0.0.1:39190...",
                "Verifying GM Access Whitelist...",
                "Scanning for Bypass Tools...",
                "Loading Anti-Cheat Enterprise AI - HWID+IP Ban...",
                "Security Layer 2 - PASSED - HWID Registered - RED Yard Ready!"
            };
            for(int i=0;i<steps.Length;i++) {
                status.Text = steps[i]; bar.Value = (i+1)*100.0/steps.Length;
                ...
                if(bar.Value<0 || bar.Value>100) {
                    ...
                    try { File.AppendAllText("ban_hwid_ip.txt", $"{DateTime.Now} BAN PERMANEN HWID:{hwid} IP:{ip} REASON: BYPASS LAYER 2\n"); } catch {}
                    MessageBox.Show($"SECURITY BREACH: Bypass Layer 2!\nHWID: {hwid}\nIP: {ip}\nAUTO BAN HWID + IP PERMANEN!\nWeb: https://ngpb.nhg.one", "PBNG ANTI-CHEAT ENTERPRISE - BAN PERMANEN", MessageBoxButton.OK, MessageBoxImage.Stop);
                    Environment.Exit(0);
                }
            }
            IsPassed = true; await Task.Delay(300); Close();
        ...
        protected override void OnClosing(System.ComponentModel.CancelEventArgs e) {
            if(!IsPassed) {
                var hwid = Environment.MachineName + "-" + Environment.UserName; var ip = "127.0.0.1";
                try { File.AppendAllText("ban_hwid_ip.txt", $"{DateTime.Now} BAN PERMANEN HWID:{hwid} IP:{ip} REASON: BYPASS LAYER 2 CLOSE\n"); } catch {}
